...
data_prep <- function(dataset) {
  ...
  final_dataset <- dataset_tbl$tokens %>% 
    ...
    transpose() %>%
    map(~{
      .x %>% map(`length<-`, MAX_LENGTH) %>% do.call(rbind, .) %>% as.matrix() %>% coalesce(0L) %>% tf$convert_to_tensor(dtype = tf$int64)
    }) %>% 
...
create_padding_mask <- function(seq) {
  ...
  shape <- seq$get_shape()$as_list()
  seq <- tf$reshape(seq, tuple(shape[1], 1L, 1L, shape[2]))
  return(seq)
}
...
evaluate <- function(inp_sentence) {
  ...
  for(i in seq_len(MAX_LENGTH)) {
    ...
    predictions <- predictions[ ,length(predictions$numpy()), ]  # (batch_size, 1, vocab_size)
    predicted_id <- tf$cast(tf$argmax(predictions, axis = -1L), tf.int32)
        
    # return the result if the predicted_id is equal to the end token
    if(predicted_id == tokenizer_en$vocab_size + 1) {
      return(list(tf$squeeze(output, axis = 0), attention_weights))
    }
  ...
  return(list(tf$squeeze(output, axis = 0L), attention_weights))
}
